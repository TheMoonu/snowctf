...
@register.simple_tag
def get_challenge_comment_count(entry):
    """获取一个文章的评论总数"""
...
@register.simple_tag
def get_challenge_parent_comments(entry):
    """获取一个文章的父评论列表，逆序只选取后面的20个评论"""
    lis = entry.challenge_comments.filter(parent=None).order_by("-id")[:20]
    return lis
...
@register.simple_tag
def get_challenge_comment_user_count(entry):
    """获取评论人总数"""
    p = []
    lis = entry.challenge_comments.all()
    for each in lis:
